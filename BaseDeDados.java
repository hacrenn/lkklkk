...
public class BaseDeDados {
    ...
    // R13
    public void salvarArtigosBinario() throws IOException {
        try (ObjectOutputStream out = new ObjectOutputStream(new FileOutputStream("arquivo.bin"))) {
            out.writeObject(artigosPorTitulo);
        }
    }

    public void carregarArtigosBinario() throws IOException, ClassNotFoundException {
        try (ObjectInputStream in = new ObjectInputStream(new FileInputStream("arquivo.bin"))) {
            artigosPorTitulo = (HashMap<String, Artigo>) in.readObject();
        }
    }
}
